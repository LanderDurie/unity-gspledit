...
{
    [BurstCompile]
    public class SplatDataCreator : EditorWindow
    {
        ...
        readonly Utils.FilePickerControl m_FilePicker = new();
        ...
        [SerializeField] SplatData.VectorFormat m_FormatPos;
        [SerializeField] SplatData.VectorFormat m_FormatScale;
        [SerializeField] SplatData.ColorFormat m_FormatColor;
        [SerializeField] SplatData.SHFormat m_FormatSH;
        ...
        bool isUsingChunks =>
            m_FormatPos != SplatData.VectorFormat.Float32 ||
            m_FormatScale != SplatData.VectorFormat.Float32 ||
            m_FormatColor != SplatData.ColorFormat.Float32x4 ||
            m_FormatSH != SplatData.SHFormat.Float32;

        [MenuItem("Tools/Gaussian Splats/Create SplatData")]
        public static void Init()
        {
            var window = GetWindowWithRect<SplatDataCreator>(new Rect(50, 50, 360, 340), false, "Gaussian Splat Creator", true);
        ...
        void OnGUI()
        {
            ...
            {
                Utils.PLYFileReader.ReadFileHeader(m_InputFile, out m_PrevVertexCount, out var _, out var _);
            ...
            if (m_PrevVertexCount > 0)
            {
                sizePos = SplatData.CalcPosDataSize(m_PrevVertexCount, m_FormatPos);
                sizeOther = SplatData.CalcOtherDataSize(m_PrevVertexCount, m_FormatScale);
                sizeCol = SplatData.CalcColorDataSize(m_PrevVertexCount, m_FormatColor);
                sizeSHs = SplatData.CalcSHDataSize(m_PrevVertexCount, m_FormatSH);
                long sizeChunk = isUsingChunks ? SplatData.CalcChunkDataSize(m_PrevVertexCount) : 0;
        ...
        void ApplyQualityLevel()
        {
            switch (m_Quality)
            {
                ...
                case DataQuality.VeryLow: // 18.62x smaller, 32.27 PSNR
                    m_FormatPos = SplatData.VectorFormat.Norm11;
                    m_FormatScale = SplatData.VectorFormat.Norm6;
                    m_FormatColor = SplatData.ColorFormat.BC7;
                    m_FormatSH = SplatData.SHFormat.Cluster4k;
                    break;
                case DataQuality.Low: // 14.01x smaller, 35.17 PSNR
                    m_FormatPos = SplatData.VectorFormat.Norm11;
                    m_FormatScale = SplatData.VectorFormat.Norm6;
                    m_FormatColor = SplatData.ColorFormat.Norm8x4;
                    m_FormatSH = SplatData.SHFormat.Cluster16k;
                    break;
                case DataQuality.Medium: // 5.14x smaller, 47.46 PSNR
                    m_FormatPos = SplatData.VectorFormat.Norm11;
                    m_FormatScale = SplatData.VectorFormat.Norm11;
                    m_FormatColor = SplatData.ColorFormat.Norm8x4;
                    m_FormatSH = SplatData.SHFormat.Norm6;
                    break;
                case DataQuality.High: // 2.94x smaller, 57.77 PSNR
                    m_FormatPos = SplatData.VectorFormat.Norm16;
                    m_FormatScale = SplatData.VectorFormat.Norm16;
                    m_FormatColor = SplatData.ColorFormat.Float16x4;
                    m_FormatSH = SplatData.SHFormat.Norm11;
                    break;
        ...
        unsafe void CreateAsset()
        {
            ...
            SplatData.CameraInfo[] cameras = LoadJsonCamerasFile(m_InputFile, m_ImportCameras);
            ...
            NativeArray<SplatData.SHTableItemFloat16> clusteredSHs = default;
            if (m_FormatSH >= SplatData.SHFormat.Cluster64k)
            {
            ...
            string baseName = Path.GetFileNameWithoutExtension(Utils.FilePickerControl.PathToDisplayString(m_InputFile));

            EditorUtility.DisplayProgressBar(kProgressTitle, "Creating data objects", 0.7f);
            SplatData asset = ScriptableObject.CreateInstance<SplatData>();
        ...
        unsafe NativeArray<InputSplatData> LoadPLYSplatFile(string plyPath)
        {
            ...
            try
            {
                Utils.PLYFileReader.ReadFile(plyPath, out splatCount, out vertexStride, out _, out verticesRawData);
            }
            catch (Exception ex)
            {
        ...
        struct ReorderMortonJob : IJobParallelFor
        {
            ...
            public void Execute(int index)
            {
                ...
                ulong code = GSUtils.MortonEncode3(ipos);
        ...
        struct ConvertSHClustersJob : IJobParallelFor
        {
            ...
            public NativeArray<SplatData.SHTableItemFloat16> m_Output;
            public void Execute(int index)
            {
                var addr = index * 15;
                SplatData.SHTableItemFloat16 res;
        ...
        static unsafe void ClusterSHs(NativeArray<InputSplatData> splatData, SplatData.SHFormat format, out NativeArray<SplatData.SHTableItemFloat16> shs, out NativeArray<int> shIndices)
        {
            shs = default;
            shIndices = default;

            int shCount = SplatData.GetSHCount(format, splatData.Length);
            ...
            {
                SplatData.SHFormat.Cluster64k => 0.3f,
                SplatData.SHFormat.Cluster32k => 0.4f,
                SplatData.SHFormat.Cluster16k => 0.5f,
                SplatData.SHFormat.Cluster8k => 0.8f,
                SplatData.SHFormat.Cluster4k => 1.2f,
            ...
            Utils.KMeansClustering.Calculate(kShDim, shData, kBatchSize, passesOverData, ClusterSHProgress, shMeans, shIndices);
            shData.Dispose();

            shs = new NativeArray<SplatData.SHTableItemFloat16>(shCount, Allocator.Persistent);
        ...
        struct LinearizeDataJob : IJobParallelFor
        {
            ...
            public void Execute(int index)
            {
                ...
                // rot
                var q = splat.rot;
                var qq = GSUtils.NormalizeSwizzleRotation(new float4(q.x, q.y, q.z, q.w));
                qq = GSUtils.PackSmallest3Rotation(qq);
                splat.rot = new Quaternion(qq.x, qq.y, qq.z, qq.w);

                // scale
                splat.scale = GSUtils.LinearScale(splat.scale);

                // color
                splat.dc0 = GSUtils.SH0ToColor(splat.dc0);
                splat.opacity = GSUtils.Sigmoid(splat.opacity);
        ...
        struct CalcChunkDataJob : IJobParallelFor
        {
            ...
            public NativeArray<SplatData.ChunkInfo> chunks;
            ...
            public void Execute(int chunkIdx)
            {
                ...
                int splatBegin = math.min(chunkIdx * SplatData.kChunkSize, splatData.Length);
                int splatEnd = math.min((chunkIdx + 1) * SplatData.kChunkSize, splatData.Length);
                ...
                for (int i = splatBegin; i < splatEnd; ++i)
                {
                    ...
                    s.opacity = GSUtils.SquareCentered01(s.opacity);
                    splatData[i] = s;
                ...
                // store chunk info
                SplatData.ChunkInfo info = default;
        ...
        {
            int chunkCount = (splatData.Length + SplatData.kChunkSize - 1) / SplatData.kChunkSize;
            ...
            fs.Write(job.chunks.Reinterpret<byte>(UnsafeUtility.SizeOf<SplatData.ChunkInfo>()));
        ...
        struct ConvertColorJob : IJobParallelFor
        {
            ...
            public SplatData.ColorFormat format;
            ...
            public unsafe void Execute(int y)
            {
                ...
                for (int x = 0; x < width; ++x)
                {
                    ...
                    switch (format)
                    {
                        case SplatData.ColorFormat.Float32x4:
                        {
                            *(float4*) dstPtr = pix;
                        }
                            break;
                        case SplatData.ColorFormat.Float16x4:
                        {
                            half4 enc = new half4(pix);
                            *(half4*) dstPtr = enc;
                        }
                            break;
                        case SplatData.ColorFormat.Norm8x4:
                        {
        ...
        static unsafe void EmitEncodedVector(float3 v, byte* outputPtr, SplatData.VectorFormat format)
        {
            switch (format)
            {
                case SplatData.VectorFormat.Float32:
                {
                    *(float*) outputPtr = v.x;
                    ...
                    *(float*) (outputPtr + 8) = v.z;
                    ...
                    break;
                case SplatData.VectorFormat.Norm16:
                {
                    ...
                    *(ushort*) (outputPtr + 4) = (ushort) (enc >> 32);
                    ...
                    break;
                case SplatData.VectorFormat.Norm11:
                {
                    uint enc = EncodeFloat3ToNorm11(math.saturate(v));
                    *(uint*) outputPtr = enc;
                }
                    break;
                case SplatData.VectorFormat.Norm6:
                {
        ...
        struct CreatePositionsDataJob : IJobParallelFor
        {
            ...
            public SplatData.VectorFormat m_Format;
        ...
        struct CreateOtherDataJob : IJobParallelFor
        {
            ...
            public SplatData.VectorFormat m_ScaleFormat;
            public int m_FormatSize;
            [NativeDisableParallelForRestriction] public NativeArray<byte> m_Output;
            ...
            public unsafe void Execute(int index)
            {
                ...
                outputPtr += SplatData.GetVectorSize(m_ScaleFormat);
        ...
        {
            int dataLen = inputSplats.Length * SplatData.GetVectorSize(m_FormatPos);
            ...
            CreatePositionsDataJob job = new CreatePositionsDataJob
            {
                ...
                m_FormatSize = SplatData.GetVectorSize(m_FormatPos),
                m_Output = data
            };
        ...
        {
            int formatSize = SplatData.GetOtherSizeNoSHIndex(m_FormatScale);
        ...
        {
            uint2 xy = GSUtils.DecodeMorton2D_16x16(idx);
            uint width = SplatData.kTextureWidth / 16;
            idx >>= 8;
            uint x = (idx % width) * 16 + xy.x;
            uint y = (idx / width) * 16 + xy.y;
            return (int)(y * SplatData.kTextureWidth + x);
        }
        ...
        {
            var (width, height) = SplatData.CalcTextureSize(inputSplats.Length);
            ...
            GraphicsFormat gfxFormat = SplatData.ColorFormatToGraphics(m_FormatColor);
        ...
        {
            [ReadOnly] public NativeArray<InputSplatData> m_Input;
            public SplatData.SHFormat m_Format;
            ...
            public unsafe void Execute(int index)
            {
                ...
                switch (m_Format)
                {
                    case SplatData.SHFormat.Float32:
                    {
                        SplatData.SHTableItemFloat32 res;
                        res.sh1 = splat.sh1;
                        res.sh2 = splat.sh2;
                        res.sh3 = splat.sh3;
                        res.sh4 = splat.sh4;
                        ...
                        res.shF = splat.shF;
                        res.shPadding = default;
                        ((SplatData.SHTableItemFloat32*) m_Output.GetUnsafePtr())[index] = res;
                    }
                        break;
                    case SplatData.SHFormat.Float16:
                    {
                        SplatData.SHTableItemFloat16 res;
                        ...
                        res.shPadding = default;
                        ((SplatData.SHTableItemFloat16*) m_Output.GetUnsafePtr())[index] = res;
                    }
                        break;
                    case SplatData.SHFormat.Norm11:
                    {
                        SplatData.SHTableItemNorm11 res;
                        ...
                        ((SplatData.SHTableItemNorm11*) m_Output.GetUnsafePtr())[index] = res;
                    }
                        break;
                    case SplatData.SHFormat.Norm6:
                    {
                        SplatData.SHTableItemNorm6 res;
                        ...
                        res.shPadding = default;
                        ((SplatData.SHTableItemNorm6*) m_Output.GetUnsafePtr())[index] = res;
                    }
                        break;
                    default:
                        break;
        ...
        void CreateSHData(NativeArray<InputSplatData> inputSplats, string filePath, ref Hash128 dataHash, NativeArray<SplatData.SHTableItemFloat16> clusteredSHs)
        {
            ...
            {
                int dataLen = (int)SplatData.CalcSHDataSize(inputSplats.Length, m_FormatSH);
                NativeArray<byte> data = new(dataLen, Allocator.TempJob);
        ...
        static SplatData.CameraInfo[] LoadJsonCamerasFile(string curPath, bool doImport)
        {
            if (!doImport)
                return null;
            ...
            var jsonCameras = Utils.JSONParser.FromJson<List<JsonCamera>>(json);
            if (jsonCameras == null || jsonCameras.Count == 0)
                return null;

            var result = new SplatData.CameraInfo[jsonCameras.Count];
            for (var camIndex = 0; camIndex < jsonCameras.Count; camIndex++)
            {
                ...
                var cam = new SplatData.CameraInfo
                {
                    pos = pos,
                    axisX = axisx,
                    axisY = axisy,
